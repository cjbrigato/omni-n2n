...
// ####################################################################################
#ifdef ARG0_COMMUNITY
    strncpy((char *)conf.community_name, basename(argv[0]), N2N_COMMUNITY_SIZE);
#else
...
// #####################################################################################
    ...
    while (runlevel < 5)
    {
        ...
        if (runlevel == 1)
        { /* PING has been sent to all known supernodes */
            if (eee->sn_pong)
            {
                // first answer
                eee->sn_pong = 0;
        ...
        if (runlevel == 2)
        { /* send REGISTER_SUPER to get auto ip address from a supernode */
